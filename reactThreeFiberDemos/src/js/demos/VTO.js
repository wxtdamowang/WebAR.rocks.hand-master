...
const VTO = () => {
  ...
  const canvasRef = useRef(null); // use this ref to get the canvas reference
  ...
  // };
  const takeScreenshot = () => {
    const threeCanvas = canvasRef.current; // The canvas from @react-three/fiber
    const videoCanvas = canvasVideoRef.current; // The canvas showing the video feed

    // Create a new canvas that will be the size of the final screenshot
    const screenshotCanvas = document.createElement("canvas");
    screenshotCanvas.width = sizing.width;
    screenshotCanvas.height = sizing.height;
    const ctx = screenshotCanvas.getContext("2d");

    // Draw the video canvas onto the screenshot canvas
    ctx.drawImage(videoCanvas, 0, 0, sizing.width, sizing.height);

    // Draw the three.js canvas (with AR content) onto the screenshot canvas
    ctx.drawImage(threeCanvas, 0, 0, sizing.width, sizing.height);

    // Convert the screenshot canvas to a data URL
    const imageDataUrl = screenshotCanvas.toDataURL();
  ...
  return (
    <div>
      ...
      <div className="VTOButtons">
        ...
        <VTOButton onClick={takeScreenshot}>Take Screenshot</VTOButton>{" "}
        {/* Add this button */}
      </div>
    </div>
  );
